...
import { debugAtom, mediaStreamActiveAtom, selectedNavItemAtom } from '@/lib/atoms';
...
import { APP_NAME, GITHUB_URL, LOGO_ALT, LOGO_IMAGE } from '@/lib/constants';
...
export function Dashboard() {
  ...
  const [selectedNavItem, setSelectedNavItem] = useAtom<NavItem>(selectedNavItemAtom);
    ...
    <div className="grid h-screen w-full pl-[53px]">
      <aside className="inset-y fixed left-0 z-20 flex h-full flex-col border-r">
        <div className="border-b p-2">
          ...
          >
            <Image src={`/${LOGO_IMAGE}`} alt={LOGO_ALT} width={28} height={28} />
          </Button>
        </div>
            ...
            <Tooltip key={index}>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className={cn('rounded-lg', selectedNavItem.ariaLabel === item.ariaLabel && 'bg-muted')}
        ...
        <div className="mt-auto grid gap-1 p-2">
          {/* HeyGen links removed. Add Azure Speech/Avatar docs or help links here if needed. */}
        </div>
      </aside>
      <div className="flex flex-col">
        <header className="sticky top-0 z-10 flex h-[53px] items-center gap-1 border-b bg-background px-4">
          ...
          <Drawer>
            ...
            <DrawerContent className="max-h-[80vh]">{selectedNavItem.content}</DrawerContent>
          </Drawer>
          <Button variant="outline" size="icon" className="ml-auto gap-1.5 text-sm" title="Share It">
        ...
        <main className="grid flex-1 gap-4 overflow-auto p-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          >
            <Badge variant="outline" className="absolute right-3 top-3 font-mono font-normal">
              {mediaStreamActive ? (
          ...
          <div className="relative hidden flex-col gap-4 lg:flex">
            {/* Chat messages panel removed - was causing infinite loops */}
          </div>
        </main>
      </div>
    </div>
